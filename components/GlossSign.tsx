...
interface GlossSignProps {
  sign: string;
  isHighlighted: boolean;
  isActive: boolean;
}

export function GlossSign({ sign, isHighlighted, isActive }: GlossSignProps) {
  // Parse the sign notation to extract modifiers
  ...
  // Clean the sign text for display
  let cleanSign = sign
    .replace(/\(2h\)|\(Rh\)|\(Lh\)|\(rs\)|\^|\<|\>/g, '')
    .trim();
    
  // Extract connected signs if present
