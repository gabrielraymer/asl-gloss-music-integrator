...
export default function ImportScreen() {
  ...
  const [sheetMusicUri, setSheetMusicUri] = useState(null);
  const [sheetMusicName, setSheetMusicName] = useState('');
  const [glossContent, setGlossContent] = useState('');
  ...
  useEffect(() => {
    if (fileUri && fileName) {
      processGlossFile(fileUri);
      
      const namePart = fileName.replace('-GLOSS.txt', '').replace(/_/g, ' ');
      setTitle(namePart);
    }
  }, [fileUri, fileName]);
  
  const processGlossFile = async (uri) => {
    try {
      setProcessing(true);
      setError(null);
